<div id="front-businesses">
	<?php if (isset($_GET['business']) && !empty($_GET['business'])) { ?>
		...
		<div class="title">
			...
			$sql = "SELECT * FROM business_timetable
					WHERE business_id = '{$business_id}'";
			$result = $mysqli->query($sql);
			$business_timetable = $result->fetch_assoc();
			...
			<table>
				<?php foreach ($days as $k => $v) { ?>
					<tr>
						<td><?php echo $k; ?></td>
						<td>
							<?php echo substr_replace($business_timetable["{$v}_start"], ":", strlen($business_timetable["{$v}_start"])-2, 0); ?>
							-
							<?php echo substr_replace($business_timetable["{$v}_end"], ":", strlen($business_timetable["{$v}_end"])-2, 0); ?>
						</td>
					</tr>
				<?php } ?>
			</table>
	...
	<?php } else { ?>
		...
		<form action = "businesses.php" method = "post" id = "businesses-search">
			<table>
				<tr>
					<td>
